...
if ($REQUEST_METHOD == "POST") $brVars = $_POST;
elseif ($REQUEST_METHOD == "GET") $brVars = $_GET;

if (!isset($ENVN)) $ENVN = $brVars["ENVN"];
if (!isset($SESS)) $SESS = $brVars["SESS"];
if (!isset($NEXT)) $NEXT = $brVars["NEXT"];
if (!isset($SSID)) $SSID = $brVars["SSID"];

extract(array_merge ($_POST, $_GET, $_COOKIE));
if (empty($UTL)) ob_start("ob_gzhandler");
if (empty($PPWD)) $PPWD = FALSE;
if (empty($NPWD)) $NPWD = FALSE;

include_once("config.inc");
if (empty($locDirInc)) $locDirInc = $locDirBase.$locEnvDir[$ENVN];
...
function user_login($username, $md5pwd, $clrpwd, $UTL, $WEB) {
	...
	if ($WEB) $usrPar["PWDT"][0]["val1"] = "W";
	elseif (!$UTL) $usrPar["PWDT"][0]["val1"] = "L";
	else $usrPar["PWDT"][0]["val1"] = "B";
	...
	global $pwdData;
	if (!$pwdData = exec_sql("I0003", "read", $usrPar)) $pwd = md5("");
	elseif ($pwdData[0]["Password"]) $pwd = $pwdData[0]["Password"];
	else $pwd = md5("");
	if (!$md5pwd) {
		if (md5($clrpwd) != $pwd)  {
			login_log($pwdRules[0], $username, $userData[0], false);
	...
	if (!$pwdData || !$pwdData[0]["Password"]) $userData[0]["nopass"] = true;
	elseif (!$userData[0]["Password_no_rule"] && $pwdRules[0]["Password_scadenza"]) {
		...
		if ((($now-$lastPwd)/86400) > $pwdRules[0]["Password_scadenza"]) $userData[0]["nopass"] = true;
	}
	if($userData[0]["Installazione"]) setG("locInstalation", $userData[0]["Installazione"]);
	return $userData[0];
}
...
function set_auth($user, $group) {
	$allAuth = array();
	$autA = array();
	if ($group) if (!$allAuth = exec_sql("I0021", "read", array("USRG"=>array(array("val1"=>$group))))) $allAuth = array();
	if ($allUser = exec_sql("I0016", "read", array("USRN"=>array(array("val1"=>$user))))) $allAuth = array_merge($allAuth, $allUser);
	if (!$allAuth) setG("autA", array("area"=>"NONE"));
	else {
		$lin=0;
		foreach ($allAuth as $linAuth) {
			if (!$linAuth["Parametro_gruppo"]) {
				if ($linAuth["Parametro_segno"] == "1") $autA[$lin]["sig"] = $linAuth["Autorizzazione_tipo"];
				else $autA[$lin]["sig"] = false;
				if ($linAuth["Area_applicativa"]) $autA[$lin]["area"] = $linAuth["Area_applicativa"];
				if ($linAuth["Processo_applicativo"]) $autA[$lin]["proc"] = $linAuth["Processo_applicativo"];
				if ($linAuth["Autorizzazione_grupo"]) $autA[$lin]["grup"] = $linAuth["Autorizzazione_grupo"];
				if ($linAuth["Azione_codice"]) $autA[$lin]["act"] = $linAuth["Azione_codice"];
				$lin++;
			}
			else {
				if ($linAuth["Parametro_segno"] == "1") $autP[$linAuth["Parametro_gruppo"]]["I"][] = $linAuth["Valore_1"];
				else $autP[$linAuth["Parametro_gruppo"]]["E"][] = $linAuth["Valore_1"];
			}
		}
		setG("autA", array_reverse ($autA, true));
		if ($autP) setG("autP", $autP);
	}
}
function check_auth_ac($area, $proc, $grup, $act) {
	$autA = getG("autA");
    $act=strval($act);
	if (!$autA || ($area == "SY" && $proc == "SYS" && $grup == "SYS") || $autA["area"] == "NONE") return "2";
	else foreach ($autA as $linA){ 
		if ((!isset($linA["act"]) || $linA["act"] == $act) && (!isset($linA["grup"]) || $linA["grup"] == $grup) && (!isset($linA["proc"]) || $linA["proc"] == $proc) && (!isset($linA["area"]) || $linA["area"] == $area)) return $linA["sig"];
	}
	return true;
}
...
function check_auth_acPdt($area, $proc, $grup, $act) {
	$autA = getG("autA");
        $act=strval($act);
	if (!$autA || $autA["area"] == "NONE") return true;
	else foreach ($autA as $linA) {
...
//			if (isset($linA["sig"])) return true;
			if (is_array($linA)) if ($linA["sig"]) return true;
			else return false;
		}
	}
	return true;
...
function destroy_sess($id, $num, $user=false) {
	...
	if (file_exists($dir) && $d = dir($dir)) {
		while($entry=$d->read()) if ($entry != "." && $entry != "..") @unlink($dir."/".$entry);
		$d->close();
...
function destroy_all_sess($SessionID) {
	...
	if ($data = exec_sql("I0011", "read", $sesPar)) foreach ($data as $rec) {
		destroy_sess($rec["Sessione_id"], $rec["Sessione_numero"]);
		if ($rec["Modificazione"]) $chkMod[$rec["Modificazione"]] = true;
	}
	if (!empty($chkMod)) chkStatusModif($chkMod);
	return true;
}
...
function wpms_session_read ($SessionID) {
    ...
    $SSID = $SessionID;
    if ($SESS == "") return "";
    if (!$_SESSION) $_SESSION = $saveGlobals;
    ...
    if ($data = exec_sql("I0011", "read", $sesPar)) {
        ...
        global $MODI;
        if ($data[0]["Modificazione"]) $MODI = $data[0]["Modificazione"];
    ...
    if (PHP_MAJOR_VERSION >= 8) return serialize($_SESSION);
}
...
function load_class($class, $instance="", $par="") {
	...
	if (empty($loaded_classes[$class])) {
		...
		if (include($dirClasses.$class.".cla")) $loaded_classes[$class] = true;
		else {
	...
	if ($instance != "" && empty($loaded_instances[$class][$instance])) {
		global ${$instance};
		if (is_array($par)) ${$instance} = new $class($par);
...
function load_class_GN($class, $instance) {
	...
	if (empty($loaded_gn_classes[$class])) {
		if (!include_once($locDirInc.$class.".cla")) {
			if ($class == "GnMsg") abort("Error loading messages");
			else send_msg("SY", "GEN", "40", array("p1"=>$class));
			return false;
		} else {
		...
		if (${$instance}->classname == $class) return true;
		else {
			if (!include_once($locDirInc.$class.".cla")) {
				if ($class == "GnMsg") abort("Error loading messages");
				else send_msg("SY", "GEN", "40", array("p1"=>$class));
...
function exec_sql($class, $method, $par="") {
	...
	if (!$UTL && !$RFS) ob_start("getBufferSql");
	if (getG("debug")) {
	...
	if (getG("debugLog") && ((!getG("debugObject") || (getG("debugObject") == $class && getG("debugMethod") == $method)))) setG("strDbgLog", true);
	elseif (getG("strDbgLog")) setG("strDbgLog", false);
	if ($par == "") $ret = ${$class}->{$method}();
	else $ret = ${$class}->{$method}($par);
	if (getG("strDbgLog") && getG("debugObject") && getG("debugObject") == $class && getG("debugMethod") == $method) setG("strDbgLog", false);
	if (!$UTL && !$RFS) ob_end_flush();
	if (getG("debug")) {
		$perfSQL = getG("perfSQL");
		if ($sqlStack[$s]["sql"]) $perfSQL[$n]["sql"] = $sqlStack[$s]["sql"];
		if ($sqlStack[$s]["inf"]) $perfSQL[$n]["inf"] = $sqlStack[$s]["inf"];
		if ($perfSQL[$n+1]) {
			$n = isr_count($perfSQL);
	...
	if ($fillW0076 = getG("W0076")) {
		setG("W0076", "");
		if (!$err) exec_sql("W0076", "write", $fillW0076);
	}
	return $ret ?? [];
}
...
function getBufferSql($buffer) {
	if ($buffer != "") {
		...
		if (strstr($buffer, " error</b>:")) return $buffer;
	}
}
function send_msg($area, $process, $code, $par="", $wf=FALSE, $pwf=array()) {
	...
	global $EVE;
	global $RFS;
	if (!$log && getG("messagesOnLog") && $area.$process.$code != "INADM7") $log = "I";
	$MSG->getMsg($area, $process, $code, $par);
	if ($catchErr) $catchErr = $MSG->msgText;
	else {
		if ($MSG->msgType != "F" && $log && (!$EVE || $EVE && $RFS) && ($log == "B" || ($log == "I" && ($MSG->msgType == "I" || $MSG->msgType == "T")) )) {
			...
			if ($repMessages) $next = isr_count($repMessages);
			else $next = 0;
		...
		if ($MSG->msgType == "C" || $MSG->msgType == "E" || $MSG->msgType == "Q") $err = true;
		if ($MSG->workFlow && !$wf) if ($callStack = debug_backtrace()) if ($nmFnct = pathinfo($callStack[0]["file"])) {
			...
			if (!empty($arFnct[1])) $wf["met"] = substr($arFnct[1], 0, 40);
		...
		if ($MSG->workFlow && $wf && $_SESSION["allWrkMsg"][$area.$process.$code]) {
			...
			if (isr_count($MSG->parM)) foreach($MSG->parM as $namPar) $parM[$namPar] = $namPar;
			if (isr_count($MSG->parO)) foreach($MSG->parO as $namPar) $parO[$namPar] = $namPar;
			if (isr_count($pwf)) foreach ($pwf as $parName => $parValue) {
				if ($parM[$parName] && $parValue) {
					unset($parM[$parName]);
					$wfEvents[$next]["pwf"][$parName] = $parValue;
				} elseif ($parO[$parName] && $parValue) $wfEvents[$next]["pwf"][$parName] = $parValue;
			}
			if (isr_count($parM)) unset($wfEvents[$next]);
	...
	if (!empty($par["j1"])) {
		...
		if ($par["j2"] != "") {
			...
			if ($par["j3"] != "") {
				...
				if ($par["j4"] != "") {
					$javaPar .= ",".$par["j4"];
					if ($par["j5"] != "") $javaPar .= ",".$par["j5"];
				}
			}
		}
	}
	$text = str_replace(array("<BR>", "<br>"), "\\n", addslashes($MSG->msgText));
	if ($MSG->msgType == "C") return "if (!new_confirm('".$text."'".$javaPar.")) return false";
	if ($MSG->msgType == "E") return "new_alert('".$text."','E'".$javaPar."); return false";
	else return "new_alert('".$text."','".$MSG->msgType."'".$javaPar.")";
}
function abort($msg) {
	global $err;
	global $repMessages;
...
function def_output($area="", $process="", $code="", $par="") {
	...
	global $WEB;
	global $repMessages;
	if ($area && $process && $code) if (load_class_GN("GnMsg", "MSG")) $MSG->getMsg($area, $process, $code, $par);
	$msgE = $msgtype = $msgCode = $msgText = FALSE;
	if ($MSG->msgType == "E") $msgE = TRUE;
	$msgType = $MSG->msgType;
	...
	if (!$msgE && isr_count($repMessages) > 1) foreach ($repMessages as $recMessages) if ($recMessages["type"] == "E") {
		$msgE = $msgType = "E";
		$msgCode = $recMessages["code"];
		$msgText = $recMessages["text"];
	}
	echo "<HTML><HEAD><TITLE>WPMS - ";
	if ($msgType == "E") echo "ERROR $msgCode";
	elseif ($msgType == "Q") echo "QUERY $msgCode";
	else echo "INFO $msgCode";
	echo "</TITLE>\n";
	if (!$UTL && !$RFS && $SESS && $SESS != "99" && $ENVN) echo "<SCRIPT SRC=\"BwFunctions.js\"></SCRIPT><SCRIPT>var session='".$SESS."';var envn='".$ENVN."';</SCRIPT></HEAD>";
	elseif ($RFS) echo "<SCRIPT>function submitForm(action,value){\n\tdocument.post.POSTACTION.value = action\n\tdocument.post.POSTVALUE.value = value\n\tdocument.post.submit()\n}\n</SCRIPT></HEAD>\n";
	$withMessageQ = FALSE;
	if (getG("locUXenv")) $locationReplace = $_SERVER["HTTP_HOST"].getG("locEnvDir")."loginPdt".getG("locUXenv").".php?&LOGOFF=RF&ENVN=".$ENVN;
	else $locationReplace = $_SERVER["HTTP_HOST"]."/loginPdt.php?&LOGOFF=RF&ENVN=".$ENVN;
	if ($msgType == "Q") {
		...
		if ($msgType == "E") echo "<TABLE BGCOLOR=#000000><TR><TD><FONT COLOR=#FFFFFF>";
		echo htmlXspecialchars(stripslashes($msgText))."<BR>";
		if ($msgType == "E") echo "</FONT></TD></TR></TABLE>";
		if ($msgType == "E" && $msgCode != "SY-GEN-8" && $SESS && $SESS != "99") echo get_msg("SY", "GEN", "8")."<BR>";
		if (!$RFS) {
			echo "<CENTER><INPUT TYPE=BUTTON VALUE=OK onClick=\"";
			if (!$UTL && $SESS && $SESS != "99" && $ENVN) echo "logoff()";
	...
	if ($withMessageQ) {
		$txtAcpVal = "";
		if ($repMessages[0]["pars"]["p4"]) if ($arAcpVal = explode("||", $repMessages[0]["pars"]["p4"])) foreach ($arAcpVal as $acpVal) $txtAcpVal .= $acpVal.",";
		echo "<SCRIPT>\nif(askdVl=prompt('".htmlXspecialchars(stripslashes($msgText))." (".substr($txtAcpVal,0,-1).")"."','".$repMessages[0]["pars"]["p5"]."')) submitForm('16',askdVl)\n</SCRIPT>";
	}
	echo "</HTML>";
	ob_end_flush();
	if ($MSG->msgType == "E") $SESS = "";
	exit;
}
...
function endLog($clear=false) {
	...
	if ($log) {
		$log = "E";
		if ($clear) $allLog = array();
	}
}
function prgBarStart() {
	global $PRGBAR;
	if (!$PRGBAR) load_class_GN("GnProgressBar", "PRGBAR");
...
function exec_print($prtName = false, $post, $thisPrtr = false, $prtCode = false, $noPrint = false) {
	...
	if (!$prtName && $prtCode){
		...
		if ($post["par"]["ENTI"] || $post["par"]["BKST"]) $parPRCD["ENTI"][0]["val1"] = ($post["par"]["ENTI"][0]["val1"]) ? $post["par"]["ENTI"][0]["val1"] : $post["par"]["BKST"][0]["val1"];
		if ($post["par"]["CMPC"]) $parPRCD["CMPC"][0]["val1"] = $post["par"]["CMPC"][0]["val1"];
		if ($post["par"]["INTL"]) $parPRCD["INTL"][0]["val1"] = $post["par"]["INTL"][0]["val1"];
		if ($post["par"]["RCST"]) $parPRCD["RCST"][0]["val1"] = $post["par"]["RCST"][0]["val1"];
		if ($RFS || $VCS || $rfACC) { // if this is from RF, get params from RF user, like warehouse and user team
			if ($rfACC["ENTI"]) $parPRCD["ENTI"][0]["val1"] = $rfACC["ENTI"];
			if ($rfACC["TEAM"]) $parPRCD["TEAM"][0]["val1"] = $rfACC["TEAM"];
		}
		if ($datPRCD = exec_sql("ACPRCD", "getAccValues", $parPRCD)) $prtName = $datPRCD[0]["val"];
		else {
			$parT0134 = array();
			$parT0134["PRTC"][0]["val1"] = $prtCode;
			if ($datT0134 = exec_sql("T0134", "read", $parT0134)) $prtName = $datT0134[0]["Stampa"];
		}
	}
	$parPRTV = array();
	...
	if ($post["par"]["ENTI"] || $post["par"]["BKST"]) $parPRTV["ENTI"][0]["val1"] = ($post["par"]["ENTI"][0]["val1"]) ? $post["par"]["ENTI"][0]["val1"] : $post["par"]["BKST"][0]["val1"];
	if ($post["par"]["CMPC"]) $parPRTV["CMPC"][0]["val1"] = $post["par"]["CMPC"][0]["val1"];
	if ($post["par"]["INTL"]) $parPRTV["INTL"][0]["val1"] = $post["par"]["INTL"][0]["val1"];
	if ($post["par"]["RCST"]) $parPRTV["RCST"][0]["val1"] = $post["par"]["RCST"][0]["val1"];
	if ($RFS || $VCS || $rfACC) { // if this is from RF, get params from RF user, like warehouse and user team
		if ($rfACC["ENTI"]) $parPRTV["ENTI"][0]["val1"] = $rfACC["ENTI"];
		if ($rfACC["TEAM"]) $parPRTV["TEAM"][0]["val1"] = $rfACC["TEAM"];
	}
	...
	if ((!$datPRTV[4]["val"] && !$noPrint) || $datPRTV[0]["val"]) {
		...
		if ($datPRTV[9]["val"] == "1") {
			...
			if($RFS || $VCS || $rfACC) {
				if($rfACC["ENTI"]) $parPRTC["ENTI"][0]["val1"] = $rfACC["ENTI"];
				if($rfACC["TEAM"]) $parPRTC["TEAM"][0]["val1"] = $rfACC["TEAM"];
			}
			$datPRTC = exec_sql("ACPRTC", "getAccValues", $parPRTC);
			if ($datPRTC[0]["val"]) exec_local_print($datPRTC[0]["val"], $post, $thisPrtr, $noPrint);
		} elseif ($dat134 && !$err) {
			if ($post["lpar"]) foreach ($post["lpar"] as $l => $linPar) {
				$allPar[$l] = $post["par"];
				foreach ($linPar as $par => $parm) $allPar[$l][$par] = $parm;
			} else $allPar[0] = $post["par"];
			if (($dat134[0]["70"] && !$datPRTV) || $datPRTV[6]["val"]) {
				if ($num = exec_sql("ACNIMP", "getAccValues")) $prtSpoolName = $num[0]["val"];
				else send_msg("SY", "GEN", "7", array("p1"=>"NIMP"));
			...
			if ($dat134[0]["90"]) {
				...
				foreach ($allPar as $parL) if ($REP->getReport($dat134[0]["90"], array("par"=>$parL))) if ($REP->nextData || $REP->nextDataH) {
					if (!$err) {
						...
						if (!$err) include($locDirInc.$dat134[0]["200"]);
						if (!$err) documentOutput($thisPrtr, $prtName, $prtFile, $prtSpoolName, false, $dat134, $datPRTV, $prtDtrPrt, $prtMail2, $prtBckAttr, $parL, $post, $prtTitle, $noPrint);
					}
				}
			} else {
		...
		} else send_msg("SY", "GEN", "15", array("p1"=>$prtName));
	}
}
function exec_local_print($prtName, $post, $thisPrtr = false, $noPrint = false) {
	...
	$rfACC = array();
        $rfACC = getG("RF_USRN");
	if($RFS || $VCS || $rfACC) {
		if($rfACC["ENTI"]) $parPRTL["ENTI"][0]["val1"] = $rfACC["ENTI"];
		if($rfACC["TEAM"]) $parPRTL["TEAM"][0]["val1"] = $rfACC["TEAM"];
	}
	...
	if ((!$datPRTL[4]["val"] && !$noPrint) || $datPRTL[0]["val"]) {
		...
		if ($datI0084 && !$err) {
			...
			foreach ($allPar as $parL) if ($REPL->getReport($datI0084[0]["90"], array("par"=>$parL))) if($REPL->nextData || $REPL->nextDataH) {
				if (!$err) {
					...
					if (!$err) include($locDirInc.$datI0084[0]["200"]);
				}
				if (!$err) documentOutput($thisPrtr, $prtName, $prtFile, $prtSpoolName, true, $datI0084, $datPRTL, $prtDtrPrt, $prtMail2, $prtBckAttr, $parL, $post,$prtTitle, $noPrint);
			}
		} else {
			if (!$post["nomsg"]) send_msg("SY", "GEN", "15", array("p1"=>$prtName));
		}
	}
}
function documentOutput($printer, $prtName, $prtFile, $prtSpoolName, $localPrint, $printDefs, $printAttribute, $prtDtrPrt, $email, $prtBckAttr, $parL, $post, $prtTitle, $noPrint) {
	...
	if ($localPrint) $saveSpool = $printAttribute[9]["val"]? $printAttribute[9]["val"] : $printDefs[0]["50"];
	else $saveSpool = $printAttribute[10]["val"]? $printAttribute[10]["val"]: $printDefs[0]["50"];
	...
	if (($printAttribute[4]["val"] || $noPrint) && $printAttribute[0]["val"]) {
		...
		if ($prtBckAttr) foreach ($prtBckAttr as $keyField => $keyVal) $outData["fld"]["I0013"][0][$keyField] = substr($keyVal, 0, 20);
		elseif (isr_count($parL)) {
			$cntKeys = 1;
			if ($parL) foreach ($parL as $parName => $parVal) $outData["fld"]["I0013"][0]["Chiave_0".$cntKeys++] = substr($parVal[0]["val1"], 0, 20);
		}
		...
		if (!$printer && getG("Printers")) {
			...
			if ($datT0134 = exec_sql("T0134", "read", $parT0134)) {
				if (($datT0134[0]["Stampa_funzione"] == "ZplPrint" || $datT0134[0]["Stampa_funzione"] == "SpecialPrint") && $printersSession["barcode"]) $printer = $printersSession["barcode"];
				elseif (($datT0134[0]["Stampa_funzione"] == "EpsPrint" || $datT0134[0]["Stampa_funzione"] == "PrintList") && $printersSession["graph"]) $printer = $printersSession["graph"];
			} else {
				//check local prints
				$parI0084=array();
				$parI0084["PRTL"][0]["val1"] = $prtName;
				if($datI0084 = exec_sql("I0084", "read", $parI0084)){
					if (($datI0084[0]["Stampa_funzione"] == "ZplPrint" || $datI0084[0]["Stampa_funzione"] == "SpecialPrint") && $printersSession["barcode"]) $printer = $printersSession["barcode"];
					elseif (($datI0084[0]["Stampa_funzione"] == "EpsPrint" || $datI0084[0]["Stampa_funzione"] == "PrintList") && $printersSession["graph"]) $printer = $printersSession["graph"];
				}
			}
		} 
		...
		if (!$printer) {
			...
			$rfACC = getG("RF_USRN");
			if($RFS || $VCS || $rfACC) {
				if($rfACC["ENTI"]) $parPRTD["ENTI"][0]["val1"] = $rfACC["ENTI"];
				if($rfACC["TEAM"]) $parPRTD["TEAM"][0]["val1"] = $rfACC["TEAM"];
			}
			if ($datPRTD = exec_sql("ACPRTD", "getAccValues", $parPRTD)) {
				$outq = $datPRTD[0]["val"];
				$printer = $datPRTD[1]["val"];
			} else send_msg("IN", "PRT", "6");
		}
		...
		if ($info[0] != "01" || $datI0012[0]["Stampante_sospesa"]) {
			...
			if ($datI0035 = exec_sql("I0035", "read_I0035V01", $parI0035)) foreach ($datI0035 as $recI0035) {
		...
		if ($datI0012[0]["FrontBack"] && $printAttribute[1]["val"]) $frtBck = true;
		else $frtBck = false;
		if ($datI0012[0]["PagesSheet"] && $printAttribute[5]["val"]) $pgPrSht = true;
		else $pgPrSht = false;
		$parI0019 = array();
		if ($outq) $parI0019["OUTQ"][0]["val1"] = $outq;
		$parI0019["PRTS"][0]["val1"] = "01"; // active queue
		if (!$datI0019 = exec_sql("I0019", "read", $parI0019)) send_msg("IN", "PRT", "5", array("p1"=>$outq));
		else $outq = $datI0019[0]["Spool_coda"];
		// send pdf document by mail if we have to
		if ($printAttribute[3]["val"] && $email) foreach ($email["recipients"] as $recipient) $SYS->sendMail($email["subject"], $recipient, $prtFile);
		if (($printDefs[0]["70"] && !$printAttribute) || $printAttribute[6]["val"]) {
			...
			if ((($printDefs[0]["40"] && !$printAttribute) || $printAttribute[8]["val"]) && $datI0019[0]["Stampante_stato"] == "01" && $info[0] == "01") {
				if ($SYS->printPdfFile($prtFile, $prtSpoolName, $printer, $datI0012[0]["Stampante_tipo"], $printAttribute[7]["val"]? $printAttribute[7]["val"]: $printDefs[0]["30"], $frtBck, $pgPrSht)) {
					if (!$post["nomsg"]){
						if($EVE || $VCS) send_msg("SY", "GEN", "101");
						else send_msg("SY", "GEN", "12", array("p1"=>$prtTitle));
					} 
					$outData["fld"]["I0013"][0]["Spool_coda_stato"] = "02";
				} else {
					if (!$post["nomsg"]) send_msg("SY", "GEN", "14", array("p1"=>$prtTitle));
					$outData["fld"]["I0013"][0]["Spool_coda_stato"] = "03";
				}
			} else {
				if (!$post["nomsg"]) {
					if($EVE || $VCS) send_msg("SY", "GEN", "101");
					else send_msg("SY", "GEN", "13", array("p1"=>$prtTitle));
				}
				$outData["fld"]["I0013"][0]["Spool_coda_stato"] = "03";
			}
			$arrayDir=array_reverse(explode('/', $prtFile));
...
function getLangBrowser() {
	...
	if ($lingue_temp[0] == "pt-BR") return "BR";
	elseif ($lingue_temp[0] <> "") return strtoupper(substr($lingue_temp[0], 0, 2));
	else return "EN";
}
...
function addSessionFile ($fileName=false, $lines=false, $mode="w") {
	if (!$dir = createSessionDir()) return false;
	if (!$fileName) {
	...
	elseif ($lines) {
		...
		foreach ($lines as $rec) fwrite($fw, $rec);
		fclose($fw);
	}
	return true;
}
function getExpression ($expr, $qts="'") {
	if (substr($expr, 0, 7) == "#STRING") return trim(substr($expr, 7));
	...
	if (substr($expr, 0, 8) == "#PHPFUNC") $expr = eval("return ".str_replace("#PHPFUNC", "", $expr).";");
	return $expr;
}
function chkLock ($obj, $key, $mode, $chkip=false) {
	...
	if ($data) {
		if ($data[0]["Sessione_id"] == $SSID && $data[0]["Sessione_numero"] == $SESS) return array("0"=>true);
		elseif ($mode == "02") return array("0"=>false,"1"=>$data[0]["Std_utente"]);
		elseif (($data[0]["Sessione_ultima"] + getG("maxLckTime")) > time()) {
			if (!$chkip) return array("0"=>false,"1"=>$data[0]["Std_utente"]);
			else {
				$parI0011 = array();
				...
				if ($datI0011 = exec_sql("I0011", "read", $parI0011)) if ($datSess = igbinary_unserialize($datI0011[0]["Sessione_dati"])) {
					if ($datSess["remIP"] != getG("remIP")) return array("0"=>false,"1"=>$data[0]["Std_utente"]);
					else {
	...
	} elseif ($mode == "02") return array("0"=>false);
...
function rlsLocks ($SSID, $SESS, $lev=0) {
	...
	if ($SESS) $lckPar["SNUM"][0]["val1"] = $SESS;
	$lock4ever = getG("lock4ever");
//	if(!getG("lock4ever")) if ($data = exec_sql("I0018", "read", $lckPar)) {
	if ($data = exec_sql("I0018", "read", $lckPar)) {
		foreach ($data as $l => $linLck) if ($linLck["Sessione_livello"] >= $lev) {
			if($lock4ever && $linLck["Oggetto_blocco"] == $lock4ever["obj"] && $linLck["Blocco_chiave"] == $lock4ever["key"]) continue;
...
function lockObject ($obj, $parKeys, $type, $post, $chkip=false) {
	global $err;
	if (!$post["lpar"]) $post["lpar"][0] = $post["par"];
	foreach ($post["lpar"] as $lin => $linPar) {
		...
		if ($err) break;
			...
			if (!$lockChecked["0"]) {
				if ($type == "02") send_msg("SY", "DBA", "49");
				else {
					if ($key != "*ALL") $msgPar["p1"] = str_replace("||", "_", $key);
					else $msgPar["p1"] = "";
	...
	if (getG("localDev")) {
		foreach ($chkMod as $mod => $x) {
			...
			if ($stat = exec_sql("T0085", "read_T0085V02", $par)) {
				...
				if ($stat[0]["20"] == "10" && $data) $newStat = "20";
				elseif ($stat[0]["20"] == "20" && !$data) $newStat = "10";
				if ($newStat) {
...
function htmlXspecialchars ($string, $ent=ENT_COMPAT, $charset='ISO-8859-15') {
	if(getG("locCharset") && getG("locCharset")!="ISO-8859-2") $charset = getG("locCharset");
		...
		if ($datI0111) {
			if (($_GET["TOKEN"] == "XXX") && $datI0111[0]["Token"] == getG("token")) { // compare token in I0111 vs I0011 to discover if previous process already finished
				if (getG("Jump2Menu")) {
					$after["MENU"] = TRUE;
					setG("allMns","");
					if ($VCS) include($locDirInc."GnMenuVoice.php");
					else include($locDirInc."GnMenuPdt.php");
				} elseif ($_SESSION["repBrowser"]) include($locDirInc.$_SESSION["repBrowser"]);
				exit;
				...
				foreach ($_GET as $var => $val) {
					if ($var != "TOKEN") echo "<INPUT TYPE=HIDDEN NAME=$var VALUE=$val>";
					else echo "<INPUT TYPE=HIDDEN NAME=\"TOKEN\" VALUE=\"XXX\">";
				}
...
function convertToTimeZone($val, $type="", $defCol=array()){
	if (!getG("DBTZ") || getG("DBTZ") == getG("TimeZone")) return $val;
...
function isr_count($par) {
        if (PHP_MAJOR_VERSION < 8) return count($par);
        else {
                if (is_array($par) || $par instanceof Countable) return count($par);
                else {
...
if ($locLibDb[$ENVN] && $locLibSys[$ENVN]) {
	if (@include($locDirInc.$locLibDb[$ENVN].".cla")) $DB = new $locLibDb[$ENVN]($locHostDb[$ENVN], $locDatabase[$ENVN], $locUserDb[$ENVN], $locPasswordDb[$ENVN]);
	if (!$DB->Link_ID) {
		$err = true;
		abort("Can\'t connect to database");
	}
	if (include($locDirInc.$locLibSys[$ENVN].".cla")) $SYS = new $locLibSys[$ENVN]();
	else {
		$err = true;
...
if (!$err) {
	...
	if ($locSuperUserSmtpOptions[$ENVN]) setG("locSuperUserSmtpOptions", $locSuperUserSmtpOptions[$ENVN]);
	...
	if (!getG("LANG")) setG("LANG", getLangBrowser());
	if (!getG("DSCT")) setG("DSCT", "2");
	...
	if (function_exists("memcache_add") && $locCacheSize[$ENVN] && !$MC) { // is going to be removed because of couchbase
		if (isset($locCacheSrvs[$ENVN])) $datI0022 = $locCacheSrvs[$ENVN];
		else $datI0022 = exec_sql("I0022", "read_I0022V02");
		if ($datI0022) {
			$MC = new Memcache;
			foreach ($datI0022 as $recI0022) $MC->addServer($recI0022["140"]?$recI0022["140"]:$recI0022["10"], ini_get("memcache.default_port"));
		}
	}
	...
	if (!empty($SITE)) setG("SITE", TRUE);
	else setG("SITE", FALSE);
	if ($SESS == "NEW" || ($UTL && !$SESS)) {
		...
		if (load_class_GN("GnMsg", "MSG")) {
			if (!empty($_SERVER["HTTP_X_FORWARDED_FOR"])) setG("remIP", $_SERVER["HTTP_X_FORWARDED_FOR"]);
			else setG("remIP", $_SERVER["REMOTE_ADDR"]);
			if (!$userData = user_login($USRN, $MPWD, $PPWD, $UTL, $WEB)) {
				...
				if ($userData["Lingua_codice"] != "") setG("LANG", $userData["Lingua_codice"]);
				if (!$RFS) setG("MNUI", $userData["Menu_codice"]);
				...
				if (!getG("MNUI") && !$UTL && !$RFS) send_msg("SY", "GEN", "50");
				elseif ($NPWD) {
					if ($RFS) {
						$LPWD = strlen($NPWD);
						if (!$NPWD || !$VPWD) send_msg("IN", "SEC", "1");
						elseif ($NPWD != $VPWD) send_msg("IN", "SEC", "2");
						else $NPWD = md5($NPWD);
					}
					...
					if ($err) $newPass = true;
						...
						if ($pwdData[0]["Password"]) {
							if ($pwdData[0]["Password_old"]) $newPwd["fld"]["I0003"][0]["Password_old"] = $pwdData[0]["Password"]."|".$pwdData[0]["Password_old"];
							else $newPwd["fld"]["I0003"][0]["Password_old"] = $pwdData[0]["Password"];
						}
			...
			} else $rules4password = false;
		}
		if (!$err) {
			
			setG("USRN", $USRN);
			setG("PRGR", $userData["Utente_adm"]);
			if ($userData["Utente_gruppo"]) setG("USRG", $userData["Utente_gruppo"]);
			...
			if($RFS || $VCS) {
				...
				if($datD0258 = exec_sql("D0258", "read_all", $parD0258)) foreach($datD0258 as $recD0258) $arrTeams[$recD0258["Ente"]][$recD0258["Squadra"]] = $recD0258["SquadraD"];
				...
				if((!$datW0004[0]["Ente"] || !$datW0004[0]["Squadra"]) && (isr_count($arrTeams) == 1 && isr_count($arrTeams[$recD0258["Ente"]]) == 1)) {
					...
					else $arrTeams = array(); // something went wrong, so the team definition cannot be right
				}
				...
				if(!getG("MNUI")) {
					if($datW0004[0]["Attivita_tutte"]) if($optrMNU = unserialize($datW0004[0]["Attivita_tutte"])) {
						...
						$mnuIN = false;
						foreach($optrMNU as $recMNU) $mnuIN .= (($mnuIN)? "','": "('").$recMNU;

						setG("DSCT", "3");
						$parD0261 = $arrOtherMNU = array();
						$parD0261["OPAC"][0]["val1"] = $mnuIN."')";
						$parD0261["OPAC"][0]["sig1"] = "IN";
						if($datD0261 = exec_sql("D0261", "read_D0261V01", $parD0261)) foreach($datD0261 as $recD0261) {
							if(!$recD0261["040D"]){
								$parI0102 = array();
								$parI0102["MENU"][0]["val1"] = $recD0261["040"];
								if($datI0102 = exec_sql("I0102", "read_I0102V01", $parI0102)) $recD0261["040D"] = $datI0102[0]["02"];
							}
							if($recD0261["010"] == $optrMNU[0]) setG("MNUI", $recD0261["040"]);
							else $arrOtherMNU[$recD0261["010"]] = array("desc"=>$recD0261["040D"], "menu_rf"=>$recD0261["040"]);
						}
						setG("DSCT", "2");
					}
					// validate main menu, assigned to mai activity
					if(!getG("MNUI") && !$arrTeams) send_msg("SY", "GEN", "50");
			...
			if ($SCRW < 700) setG("SCRW", "1");
			elseif ($SCRW < 1000) setG("SCRW", "2");
			elseif ($SCRW < 1200) setG("SCRW", "3");
			else setG("SCRW", "4");
			...
			if ($datSys=exec_sql("T0039","read_T0039V01",$parSys)) foreach($datSys as $linSys) setG(($linSys["60"])?$linSys["60"]:$linSys["10"],($linSys["50"])?$linSys["50"]:$linSys["20"]);
			setG("lastPost", 1);
			setG("fixDate", "0");
			setG("debug", false);
			setG("host", $_SERVER["SERVER_NAME"]);
			$_SESSION["allUm"] = array();
			if ($um = exec_sql("D0083", "read")) foreach ($um as $recUm) $_SESSION["allUm"][$recUm["Unita_misura"]] = $recUm["Unita_misura_decimal"];
			$parVRNT["INST"][0]["val1"] = getG("locInstalation");
			if ($datVRNT = exec_sql("ACVRNT50", "read", $parVRNT)) foreach ($datVRNT as $recVRNT) $variants[$recVRNT["Report_codice"]] = $recVRNT["Variante_codice"];
			$parVRNT["USRG"][0]["val1"] = $userData["Utente_gruppo"];
			if ($datVRNT = exec_sql("ACVRNT30", "read", $parVRNT)) foreach ($datVRNT as $recVRNT) $variants[$recVRNT["Report_codice"]] = $recVRNT["Variante_codice"];
			$parVRNT["USRN"][0]["val1"] = $USRN;
			if ($datVRNT = exec_sql("ACVRNT10", "read", $parVRNT)) foreach ($datVRNT as $recVRNT) $variants[$recVRNT["Report_codice"]] = $recVRNT["Variante_codice"];
			if ($variants) foreach ($variants as $repC => $varC) $_SESSION["variants"][$repC]["varc"] = $varC;
			if($datI0070 = exec_sql("I0070", "read")) foreach($datI0070 as $recI0070) $_SESSION["localReports"][$recI0070["Report_codice"]] = TRUE;
			if($datI0103 = exec_sql("I0103", "read")) foreach($datI0103 as $recI0103) $_SESSION["localActions"][$recI0103["Azione_codice"]] = TRUE;
			$_SESSION["allWrkMsg"] = array();
			if ($d56 = exec_sql("I0056", "read_I0056V02")) foreach ($d56 as $r56) $_SESSION["allWrkMsg"][$r56["010"].$r56["020"].$r56["030"]] = TRUE;
		...
		if (getG("TIMEZONE")) date_default_timezone_set(getG("TIMEZONE"));
		if (!empty($LOGOFF)) {
			if ($SESS) {
				destroy_sess($SSID, $SESS);
				$SESS = "";
				if ($MODI) chkStatusModif(array($MODI=>true));
			} else destroy_all_sess($SSID);
			if ($LOGOFF == "VCS"){
			...
			if ($CHGMODIF == "__NONE__") $MODI = "";
			else {
				$MODI = $CHGMODIF;
				$par["MODI"][0]["val1"] = $MODI;
				$par["XLOCK"][] = "T0085";
				$stat = exec_sql("T0085", "read", $par);
				if ($stat[0]["Status_applicativo"] != "10" && $stat[0]["Status_applicativo"] != "20") exit;
			}
			...
			if ($MODI) $chkMod[$MODI] = true;
			chkStatusModif($chkMod);
			exit;
		}
		else load_class_GN("GnMsg", "MSG");
		if ($dbgOn) send_msg("SY", "OBJ", 19);
		if (!empty($PREFS)) {
			...
			else def_output("SY", "GEN", "23");
		}
		if ($SESS != "99") setG("locSession", $SSID.$SESS);
		else $SESS = "";
	}
}

//TESTE - Voice don't need token
if ($RFS && !$VCS) checkToken($RFS,$SSID, $SESS);
...
if (!$err && !empty($NEXT)) include($locDirInc.$NEXT);
elseif (!$UTL && !$RFS && !$SCRW && $repMessages[0]["text"] && $repMessages[0]["type"] != "Q") def_output();
...
if(getG("sysLog") || getG("DBGP")) writeWPMSstat();

if (!$err && getG("cmdBatch")) {
	$allCmds = getG("cmdBatch");
	setG("cmdBatch", false);
	foreach ($allCmds as $cmd) system($cmd);
} else {
	if ($SESS) {
		ob_end_flush();
		flush();
